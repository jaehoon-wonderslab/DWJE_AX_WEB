...
**집계 구간은 전날 08:00 ~ 당일 08:00 (하루 전체).**
...
> **구간은 서버가 정합니다** — 대상일만 보내면 `periodFrom` 08:00 / `periodTo` 08:00 로 옵니다.
>
...
| 4 | 일목표 | **작성자 입력.** 없으면 목표 마스터 밑값, 둘 다 없으면 빈칸 + 회색 참고값 | `targetQty` · `targetQtyOrigin` |
...
일목표 칸의 테두리가 출처를 말합니다 — **점선**은 작성자가 그날 정한 값(`MANUAL`),
**점 찍힌 선**은 목표 마스터의 밑값(`MASTER`). 마스터 값은 덮어쓰면 그날만 바뀝니다.
...
**표에 자르기 전 전량**으로 셉니다. 표는 상위 N 종만 그리므로 합계까지 그러면 조용히 모자랍니다.

대상일 · 실적 일자(전날) · 제품 종수(표시 종수 병기) · 실적 합계 · 불량 합계 ·
주간 실적 합계 · 이 구간 미가동 종수 · 일목표 미기입 종수

대상일 구간에 실적이 없고 그 주에만 돈 제품도 행으로 나옵니다(`qty` 0, `weekQty` 만 값).
거르지 않고 「이 구간 미가동」 으로 따로 셉니다 — 조간회의에서 물어볼 항목이기 때문입니다.
...
- [x] 생성 이력 목록
- [x] 엑셀 다운로드 (양식 열 그대로)
- [x] **서버 집계 구간 08:00~08:00** — 2026-09-04 반영 (시작 시각 20:00 → 08:00 은 2026-09-16 변경)
...
- [x] **제품·공정별 일목표 마스터** — 2026-09-04 서버 반영 (`/production/day-targets` 4건)
      우선순위 저장값 > 마스터 > null. **등록 화면은 지시 대기**
