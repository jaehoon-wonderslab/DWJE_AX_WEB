...
export default function HourlyDefectPivotMatrix({
  ...
  const getCellColors = (r) => {
    ...
    if (r === null || r === undefined) {
      return { bg: 'transparent', border: theme.color.border, text: theme.color.mutedForeground, bold: false };
    }
    // 히트맵 단계 — 위험(붉은 진함) · 주의(붉은 옅음) · 적정(정보색) · 양호(정상색)
    if (r >= 4.0) {
      return { bg: theme.alpha('destructive', theme.isDark ? 0.34 : 0.22), border: theme.alpha('destructive', 0.4), text: theme.color.destructive, bold: true };
    }
    if (r >= target) {
      return { bg: theme.alpha('destructive', theme.isDark ? 0.16 : 0.1), border: theme.alpha('destructive', 0.25), text: theme.color.destructive, bold: true };
    }
    if (r >= 2.0) {
      return { bg: theme.alpha('info', theme.isDark ? 0.1 : 0.06), border: theme.alpha('info', 0.16), text: theme.color.info, bold: false };
    }
    return { bg: theme.alpha('success', theme.isDark ? 0.1 : 0.06), border: theme.alpha('success', 0.16), text: theme.color.success, bold: false };
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.topInfoRow}>
        ...
        <View style={styles.legendRow}>
          <View style={styles.legendItem}>
            <View style={[styles.legendBox, { backgroundColor: theme.alpha('success', 0.2), borderColor: theme.color.success }]} />
            <Text style={[styles.legendText, { color: theme.color.mutedForeground }]}>양호 (&lt;2.0%)</Text>
          </View>
          <View style={styles.legendItem}>
            <View style={[styles.legendBox, { backgroundColor: theme.alpha('info', 0.2), borderColor: theme.color.info }]} />
            <Text style={[styles.legendText, { color: theme.color.mutedForeground }]}>적정 (2.0~3.0%)</Text>
          </View>
          <View style={styles.legendItem}>
            <View style={[styles.legendBox, { backgroundColor: theme.alpha('destructive', 0.2), borderColor: theme.color.destructive }]} />
            <Text style={[styles.legendText, { color: theme.color.mutedForeground }]}>주의 (&gt;3.0%)</Text>
          </View>
          <View style={styles.legendItem}>
            <View style={[styles.legendBox, { backgroundColor: theme.alpha('destructive', 0.45), borderColor: theme.color.destructive }]} />
            <Text style={[styles.legendText, { color: theme.color.mutedForeground }]}>위험 (&gt;4.0%)</Text>
          </View>
        </View>
      </View>
      ...
      */}
      ...
      >
        <View style={[styles.tableWrapper, { borderColor: theme.hairline }]}>
          {/* 헤더 행 */}
          <View style={[styles.headerRow, { backgroundColor: theme.alpha('foreground', 0.05) }]}>
            <View style={[styles.thCell, styles.dateCol]}>
              <Text style={[styles.thText, { color: theme.color.foreground, fontWeight: '500' }]}>일자</Text>
            </View>
            {slots.map((sLabel, idx) => (
              <View key={idx} style={[styles.thCell, styles.slotCol]}>
                <Text style={[styles.thText, { color: theme.color.mutedForeground, fontWeight: '500' }]}>{sLabel}</Text>
              </View>
            ))}
            ...
            <View
              style={[
                ...
                styles.summaryCol,
                {
                  backgroundColor: theme.alpha('foreground', 0.08),
                  borderLeftWidth: 2,
                  borderLeftColor: theme.divider,
                },
              ]}
            >
              <Text style={[styles.thText, { color: theme.color.foreground, fontWeight: '500' }]}>
                일일 평균
              </Text>
            </View>
          </View>
            ...
            return (
              ...
              >
                ...
                <View style={[styles.tdCell, styles.dateCol]}>
                  <Text style={[styles.dateText, { color: theme.color.foreground, fontWeight: '600' }]}>
                    {row.date}
                  </Text>
                </View>
                  ...
                  if (cell.empty) {
                    return (
                      <View key={cIdx} style={[styles.tdCell, styles.slotCol, styles.heatCell, { borderColor: colors.border }]}>
                        <Text style={[styles.cellRateText, { color: theme.color.mutedForeground, fontWeight: '400' }]}>—</Text>
                      </View>
                    );
                  }
                ...
                <View
                  style={[
                    ...
                    styles.summaryCol,
                    {
                      backgroundColor: theme.alpha('foreground', 0.05),
                      borderLeftWidth: 2,
                      borderLeftColor: theme.divider,
                    },
                  ]}
                >
                  <Text
                    style={[
                      ...
                      { color: theme.color.foreground, fontWeight: '500' },
                    ]}
                  >
      ...
      <View style={styles.bottomHelp}>
        <Icon name="info" size={13} color={theme.color.mutedForeground} />
        <Text style={[styles.helpText, { color: theme.color.mutedForeground }]}>
      ...
      {hoverTooltip && typeof document !== 'undefined' && createPortal(
        <div
          style={{
            ...
            zIndex: 99999,
            pointerEvents: 'none',
            backgroundColor: theme.isDark ? 'rgba(8, 8, 8, 0.96)' : 'rgba(255, 255, 255, 0.97)',
        ...
        >
          <div style={{ fontWeight: 500, marginBottom: 6, color: theme.color.foreground, borderBottom: `1px solid ${theme.hairline}`, paddingBottom: 5 }}>
            {hoverTooltip.cell.date} [{hoverTooltip.cell.slotLabel} ~ {hoverTooltip.cell.nextSlot}]
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, marginBottom: 2 }}>
            <span style={{ color: theme.color.mutedForeground }}>불량률:</span>
            <span style={{ fontWeight: 600, color: hoverTooltip.cell.defectRate > target ? theme.color.destructive : theme.color.success }}>
              {rate(hoverTooltip.cell.defectRate)}
            </span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, marginBottom: 2 }}>
            <span style={{ color: theme.color.mutedForeground }}>투입 수량:</span>
            <span style={{ fontWeight: 500 }}>{num(hoverTooltip.cell.inputQty, ' EA')}</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, marginBottom: 2 }}>
            <span style={{ color: theme.color.mutedForeground }}>양품 / 불량:</span>
            <span>{num(hoverTooltip.cell.okQty)} / <span style={{ color: hoverTooltip.cell.ngQty > 0 ? theme.color.destructive : 'inherit' }}>{num(hoverTooltip.cell.ngQty)}</span> EA</span>
          </div>
