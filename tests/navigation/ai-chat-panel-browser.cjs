...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.getByPlaceholder('LOT · 품목 · 금형 검색').count(), 0);

    await page.getByRole('button', { name: 'AI 채팅 열기' }).click();
    await page.getByLabel('AI 채팅 작업 패널').waitFor();
    ...
    assert.equal(await page.getByText('계정·권한·기준값·연동 이력 등 운영 설정 화면을 선택합니다.', { exact: true }).count(), 1);
    ...
    await page.getByText('계정·권한·기준값·연동 이력 등 운영 설정 화면을 선택합니다.', { exact: true }).waitFor({ state: 'hidden' });

    await page.getByRole('button', { name: 'AI 채팅 닫기', exact: true }).first().click();
    await page.getByRole('button', { name: 'AI 채팅 열기', exact: true }).waitFor();
    await input.waitFor({ state: 'hidden' });
    await page.getByText('계정·권한·기준값·연동 이력 등 운영 설정 화면을 선택합니다.', { exact: true }).waitFor();
    assert.equal(page.url(), `${WEB}/menu/system`);
