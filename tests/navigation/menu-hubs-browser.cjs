...
const groups = [
  ...
  ['system', '시스템관리', '16', ['계정 관리', '메뉴 접근 권한', '데이터 접근 권한', '이상 알림 발송 조건 관리', '알림 수신자 관리', '용어 사전 관리', '제품군 순위 관리', '자연어 질의 이력', '보안 감사 로그', 'AI 모델 설정', 'AI 모델 버전 관리', 'Agent 실행 현황', '지표 측정 데이터 관리', '보고서 다운로드 이력', '업로드 문서 목록', '데이터 연동 이력']],
];
