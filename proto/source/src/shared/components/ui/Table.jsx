...
export default function Table({
  ...
  const registerCell = (entry) => {
    ...
    queueMicrotask(() => {
      ...
      pendingRef.current = [];
      // 표가 다시 그려져 떨어져 나간 칸은 버립니다
      setCells((prev) => [...prev.filter((c) => c.el.isConnected), ...fresh]);
    });
  };
    ...
    () => {
      ...
      return columns.map((col, i) => {
        ...
        const base = {
          ...
          headerFilter: filterable && !col.render ? 'input' : false,
        ...
        if (col.render) {
          base.formatter = (cell) => {
            ...
            registerCell({ id: `c${(cellSeq += 1)}`, el, colIndex: i, rowIdx: data.__idx, data });
            return el;
          };
        } else {
          base.formatter = (cell) => {
