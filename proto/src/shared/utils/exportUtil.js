...
export async function downloadFromServer({ path, body = {}, name = '파일' }) {
  ...
  tables.forEach((table, index) => {
    const sheet = workbook.addWorksheet(`데모 데이터 ${index + 1}`);
    sheet.addRow([...table.querySelectorAll('.tabulator-header .tabulator-col-title')].map(cell => cell.textContent));
    table.querySelectorAll('.tabulator-row:not(.tabulator-calcs)').forEach(row => {
      sheet.addRow([...row.querySelectorAll('.tabulator-cell')].map(cell => cell.textContent));
    });
    sheet.columns.forEach(column => { column.width = 24; });
  });
  if (!tables.length) workbook.addWorksheet('데모').addRow([name, '샘플 데이터']);
  saveBlob(new Blob([await workbook.xlsx.writeBuffer()], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' }), `${name}.xlsx`);
  return true;
}
