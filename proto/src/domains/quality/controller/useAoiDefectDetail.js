...
export function useAoiDefectDetail(serialKey) {
  ...
  /**
   * 표에 담을 회차 — `ng`(불량만, 기본) · `all`(전부).
   *
   * 기본을 불량만으로 두는 이유는 한 시리얼의 회차가 2,000~3,800개라 전부 받으면 모달이 감당하지 못해서입니다.
   * 다만 **그 상태에서는 표의 「판정」이 전부 불량**입니다. 원천을 직접 조회해 본 사람이 이 표를 보면
   * PASSED 가 뒤집힌 것처럼 읽힙니다(원천은 '1'=합격이 8할입니다). 그래서 무엇을 보고 있는지 화면이 밝히고,
   * 눌러서 전체로 바꿀 수 있게 둡니다.
   */
  const [only, setOnly] = useState('ng');

  const { data, loading } = useAsync(() => loadAoiDefect(serialKey, { only }), [serialKey, only], { skip: !serialKey });
  ...
  const { seqRows, valueCols } = useMemo(() => {
    ...
    (detail?.items || []).forEach((it) => {
      ...
      // Tabulator formatter 가 읽습니다 — Set 보다 배열이 안전합니다(직렬화·복제를 타도 살아남습니다)
      row._viol = [...viol];
      rows.push(row);
    });
  ...
  return { detail, loading, images, activeImage, activeIndex, showImage, stepImage, copyPath, seqRows, valueCols, specByNo, only, setOnly };
}
